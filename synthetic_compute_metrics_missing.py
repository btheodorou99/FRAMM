...
def compute_test_metrics(net, K, M):
	...
	for batch in test_dataloader:
		...
		inv_mask = torch.zeros(len(trial), M, 4).to(device)
		inv_mask[:,:,3] = 1
...
if os.path.exists('synthetic_save/metrics_noHist.pkl'):
	metrics = pickle.load(open('synthetic_save/metrics_noHist.pkl', 'rb'))
else:
	metrics = {}
...
pickle.dump(metrics, open('synthetic_save/metrics_noHist.pkl', 'wb'))