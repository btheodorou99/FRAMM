...
def validate(val_dataloader, net, K, device, lam, final_layer, num_inv):
    ...
    for batch in val_dataloader:
        ...
        #avg_overlap = sum(num_overlap)/sum(num_tot) if sum(num_tot) != 0 else 0
    return np.nanmean(rewards), np.nanmean(rel_dif)#, avg_overlap
  
def train_policy(num_epochs, claim_seq_len, trial_seq_len, trial_dim, static_dim, dx_dim, rx_dim, lstm_dim, embedding_dim, num_layers, hidden_dim, n_heads, M, train_dataloader, val_dataloader, K, final_layer, lr, lam):
    ...
    AUTO_PATH = 'synthetic_save/UnifiedRepresentationNetwork_Auto.pt'
    mdl = torch.load(AUTO_PATH)
    net.load_state_dict(mdl['model_state_dict'])
    optimizer = torch.optim.Adam(net.parameters(), lr=lr)

    max_val_reward = -1e5
    PATH = 'synthetic_save/UnifiedRepresentationNetwork' + '_M_' + str(M) + '_K_' + str(K) + '_lam_' + str(lam) + '.pt'
...
for M in combos:
    ...
    inv_masks_file = f'synthetic_data/inv_masks_M{M}_train.pt'
    ...
    for K in combos[M]:
        for lam in combos[M][K]:
            ...
            train_policy(num_epochs, claim_seq_len, trial_seq_len, trial_dim, static_dim, dx_dim, rx_dim, lstm_dim, embedding_dim, num_layers, hidden_dim, n_heads, M, train_dataloader, val_dataloader, K, final_layer, lr, lam)