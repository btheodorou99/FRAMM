...
def validate(val_dataloader, net, K, device, lam, final_layer, num_inv):
    ...
    for batch in val_dataloader:
        ...
        _, eth_ind = torch.sort(torch.mean(eth_label, dim=1), descending=True)
        ...
        _, reward, _, _ = loss_function_os(score.to(device, non_blocking=True), label, eth_label, eth_ind, final_layer, lam, K)
...
def train_policy(num_epochs, trial_dim, static_dim, dx_dim, rx_dim, lstm_dim, embedding_dim, num_layers, hidden_dim, n_heads, M, train_dataloader, val_dataloader, K, final_layer, lr, lam):
    ...
    max_val_reward = -1e5
    PATH = 'synthetic_save/PGOS' + '_M_' + str(M) + '_K_' + str(K) + '_lam_' + str(lam) + '.pt'
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for batch in train_dataloader:
            ...
            loss, reward, util, fair = loss_function_os(score.to(device, non_blocking=True), label, eth_label, eth_ind, final_layer, lam, K)
