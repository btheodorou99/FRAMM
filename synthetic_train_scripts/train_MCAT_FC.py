...
# train functions
def validate(val_dataloader, net, K, device, lam, final_layer, M):
    net.eval()
    rel_dif = []
    num_overlap = []
    num_tot = []
...
#        avg_overlap = sum(num_overlap)/sum(num_tot) if sum(num_tot) != 0 else 0
    return np.nanmean(rewards), np.nanmean(rel_dif)
...
def train_policy(num_epochs, trial_dim, static_dim, dx_dim, rx_dim, lstm_dim, embedding_dim, num_layers, hidden_dim, n_heads, M, train_dataloader, val_dataloader, K, final_layer, lr, lam):
    ...
    max_val_reward = -1e5
    PATH = 'synthetic_save/MCAT_FC' + '_M_' + str(M) + '_K_' + str(K) + '_lam_' + str(lam) + '.pt'
