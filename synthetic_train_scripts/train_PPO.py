...
def train_policy(num_epochs, trial_dim, static_dim, dx_dim, rx_dim, lstm_dim, embedding_dim, num_layers, hidden_dim, n_heads, M, train_dataloader, val_dataloader, K, final_layer, lr, lam):
    ...
    max_val_reward = -1e5
    PATH = 'synthetic_save/PPO' + '_M_' + str(M) + '_K_' + str(K) + '_lam_' + str(lam) + '.pt'
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for batch in train_dataloader:
            ...
            score = net(trial, inv_static, inv_dx, inv_dx_len, inv_rx, inv_rx_len, inv_enroll, inv_enroll_len, inv_mask, M).squeeze(-1)
            score_old = net_old(trial, inv_static, inv_dx, inv_dx_len, inv_rx, inv_rx_len, inv_enroll, inv_enroll_len, inv_mask, M).squeeze(-1)
      
            if not torch.isfinite(score).all() or not torch.isfinite(score_old).all():
                continue
      
            loss, reward = loss_function_PPO(score.to(device, non_blocking=True), score_old.to(device, non_blocking=True), label, eth_label, final_layer, lam, K)
